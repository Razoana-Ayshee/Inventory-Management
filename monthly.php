...
<body style=" background: linear-gradient(to bottom left, #000066 0%, #ff99cc 100%);">
	...
	<div style="font-size: 20px;margin: 10px;color: orange;">Enter a Month: </div><input id="text" type="text" name="monthly"><br><br>
...
<td>
<h1>monthly purchase transaction</h1>
...
if($_SERVER['REQUEST_METHOD'] =="POST")
	{
		...
		$sql = "SELECT * FROM purchase where month='$monthly'";
$result = mysqli_query($connection, $sql);


echo "<tr>";
echo "<th>purchase id</th><th>Product Name</th><th>Quantity</th><th>purchase month</th><th>Price</th>";
echo "</tr>";
...
if (mysqli_num_rows($result) > 0) {
    ...
    while($row = mysqli_fetch_assoc($result)) {
		...
		$row5 = mysqli_fetch_assoc($result5);
			...
			if($flag==true){
				$sql6 = "SELECT * FROM purchase where pro_id='$p_name' && month='$monthly'";
...
<td>
<h1>monthly Sale transaction</h1>
...
if($_SERVER['REQUEST_METHOD'] =="POST")
	{
		...
		$monthly = $_POST['monthly'];
$sql = "SELECT * FROM sale where sale_month='$monthly'";
$result = mysqli_query($connection, $sql);


echo "<tr>";
echo "<th>Sale id</th><th>Product Name</th><th>Sale Quantity</th><th>Sale month</th><th>Sale Price</th>";
echo "</tr>";
...
if (mysqli_num_rows($result) > 0) {
    ...
    while($row = mysqli_fetch_assoc($result)) {
		...
		$row5 = mysqli_fetch_assoc($result5);
			...
			if($flag==true){
				$sql8 = "SELECT * FROM sale where pro_id='$pro_id' && sale_month='$monthly'";
...
if($total_income>$total_expense){
	...
	echo "<FONT COLOR=black style='font-size:30px'> total profit :".$tot_pro. "</FONT><br>";
	echo "<FONT COLOR=black style='font-size:30px'> total loss : ".$tot_loss."</FONT><br>";
}
