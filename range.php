...
<body style=" background: linear-gradient(to bottom left, #000066 0%, #ff99cc 100%);">
	...
	<div style="font-size: 20px;margin: 10px;color: orange;">Enter Start Date: </div><input id="text" type="DATE" name="date1">
	<div style="font-size: 20px;margin: 10px;color: orange;">Enter Finish Date: </div><input id="text" type="DATE" name="date2"><br><br>
...
<td>
<h1>duration purchase transaction</h1>
...
if($_SERVER['REQUEST_METHOD'] =="POST")
	{
		...
		$sql = "SELECT * FROM purchase where date>='$date1' AND date<='$date2' ";
...
if (mysqli_num_rows($result) > 0) {
    ...
    while($row = mysqli_fetch_assoc($result)) {
		...
		$row5 = mysqli_fetch_assoc($result5);
			...
			if($flag==true){
				$sql6 = "SELECT * FROM purchase where pro_id='$p_name' && date>='$date1' AND date<='$date2' ";
...
<td>
<h1>duration Sale transaction</h1>
...
if (mysqli_num_rows($result) > 0) {
    ...
    while($row = mysqli_fetch_assoc($result)) {
		...
		$row5 = mysqli_fetch_assoc($result5);
			...
			if($flag==true){
				$sql8 = "SELECT * FROM sale where pro_id='$pro_id' && sale_date>='$date1' AND sale_date<='$date2' ";
